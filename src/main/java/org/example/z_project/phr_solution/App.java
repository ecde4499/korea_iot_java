...
/*
    ...
    1. 프로젝트 개요
        - 개인 겅강 기록(PHR) 솔루션을 위한 간단한 콘솔 기반
    ...
    - entity: 데이터 저장소(DB)의 테이블과 매핑(연결)되는 도메인 객체
     ...
     - App.java: 프로그램 실행에 대한 진입점, 사용자와의 상호작용을 관리하고 전체 흐름을 제어
            => App.java가 사용자 입력 처리, 메뉴 출력, 흐름 제어 모두를 담당 -> SRP 위반 (단일 책임 원칙)
...
public class App {
    ...
    private static boolean processChoice(int choice) {
        switch (choice) {
            ...
            // 건강 기록 관련 기능
            case 6: {
                RecordCreateRequestDto dto = InputHandler.recordCreateRequestDto();
            ...
            case 7: {
                List< RecordListResponseDto> records = healthRecordController.getAllRecord();
