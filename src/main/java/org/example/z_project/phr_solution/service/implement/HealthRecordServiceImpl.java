...
public class HealthRecordServiceImpl implements HealthRecordService {
    ...
    public List<RecordListResponseDto> getAllRecord() {
        List<RecordListResponseDto> responseDtos = null;

        try {
            List<HealthRecord> records = recordRepository.findAll();

            responseDtos = records.stream()
                    .map(record -> new RecordListResponseDto(record.getId(), record.getPatientId()
                    , record.getDateOfVisit(), record.getDiagnosis(), record.getTreatment(), new Date()
                    ))
