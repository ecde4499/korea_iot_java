...
class InvalidUserException extends Exception {
    InvalidUserException(String massage){
        super(massage); // Exception 클래스의 getMessage() 메서드 사용 가능
    }

}

class CoustomException extends Exception {
    CoustomException (String massage) {
        super(massage);
    }
}
...
public class Exception03 {
    ...
    static void exceptions (int num) throws CoustomException, ArithmeticException, ArrayIndexOutOfBoundsException {
        if (num == 1) {
            throw new CoustomException("1번 선택 - 커스텀 예외");
        } else if (num == 2) {
