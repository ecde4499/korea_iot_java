...
// 1. 추상 클래스 정의
// : 미완성 클래스(설계도), 완전하지 않은 클래스 / 일부 구현이 누락된 불완전한 클래스
// => 자체로 인스턴스를 생성할 수 없음
...
class Police extends Human {
    // 클래스 'Police'은(는) abstract로 선언되거나 'Human'에서 추상 메서드 'work()'을(를) 구현해야 합니다.
    @Override
    void work() { // 추상 메서드 구현
...
public class A_Abstract {
    public static void main(String[] args) {
        // Human human = new Human(); // Human은(는) abstract 이며, 인스턴스화할 수 없습니다
        // => 추상 클래스 직접적인 인스턴스화가 불가능!
