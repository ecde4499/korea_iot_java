...
interface PlayingCard {
    ...
    public abstract String gerCardNumber();
    String getCardKind ();

    // 3. 디폴트 메서드
    // : 인터페이스에서 새오룬 기능을 추가하면서 기존 구현을 포함
...
class Card implements PlayingCard {
    ...
    Card (String cardKind, String cardNumber) {
        this.cardKind = cardKind;
        this.cardNumber = cardNumber;
    }

    // cf) 인터페이스 안에 추상 메서드를 구현한 클래스 메서드는 public 지정
    @Override
...
public class B_Interface {
    public static void main(String[] args) {
        ...
        System.out.println(card.getCardKind()); // 7
        System.out.println(card.gerCardNumber()); // HEART
