...
public class StudentView {
    ...
    public void showMenu() {
        while (true) {
            try {
                ...
                int choice = Integer.parseInt(sc.nextLine()); // 버퍼처리를 안해도 된다. (숫자를 문자로 변경)
                ...
                switch (choice) {
                    ...
                    case 6 -> {
                        // switch - case 화살표 문법에서 여러 줄의 코드 작성 시 {} 중괄호로 범위를 감싸야 한다!
                        System.out.println("프로그램을 종료합니다.");
                        return; // 해당 메서드(showMenu) 죵료
                    }
    ...
    private void showAllStudentsView() {
        ...
        if (list.isEmpty()) {
            ...
            for (StudentResponseDto dto: list) {
                System.out.println(dto); // toString을 재정의 하여 주소값이 아닌 데이터값 출력
            }
        }
    }
    ...
    private void updateStudentView() {
        ...
        StudentResponseDto dto1 = controller.getStudentById(studentNumber);
