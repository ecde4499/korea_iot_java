...
public class Practice03 {
    ...
    static class UserRepository {
        private final Map<String, User> userOb = new HashMap<>();

        // 생성자에 기본 사용자 추가
        public UserRepository() {
            userOb.put("user01", new User("user01", "이승아"));
            userOb.put("user02", new User("user02", "이도경"));
        }

        // Optional을 반환하는 조회 메서드
        public Optional<User> findById(String id) {
            return Optional.ofNullable(userOb.get(id));
        }
    }
    public static void main(String[] args) {
        ...
        try {
            User user4 = repo.findById("user484")
