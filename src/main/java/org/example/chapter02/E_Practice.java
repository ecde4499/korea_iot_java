...
public class E_Practice {
    public static void main(String[] args) {
		...
		*/
        ...
        System.out.println("== 점수에 따른 학점 출력 ==");
        System.out.print("점수 입력 >> ");
        int num = sc.nextInt();

        if (num > 100 || num < 0) System.out.println("유효하지 않은 점수입니다.");
        ...
        System.out.println("== 삼각형 모양의 별(*) 출력 ==");
        for (int i = 0; i <= 4; i++) {
            // i : 행수 (1행 부터 5행까지 반복)
            for (int j = 0; j <= i; j++) {
                // 1행은 별 1개, 2행은 별 2개... 5행은 별 5개
                System.out.print("*");
            }
        ...
        *****
        ...
        +) 공백은 4부터 0까지 -1씩 감소 => rows - 1 (조건)
        +) 별은 1부터 5까지 1씩 증가 => 행을 초과하지 않도록 반복
         */
        System.out.println("== 역삼각형 모양의 별(*) 출력 ==");
        int rows = 5; // 출력할 줄의 수
