...
class Pair<K, V> {
    ...
    // [접근제한자] <타입매개변수> 반환타입 메서드명(매개변수...) { 구현부 }
    // cf) static 멤버에서는 클래스의 제너릭 타입 매개 변수 사용이 불가
    //      => static 메서드는 제너릭 타입을 직접 선언(<T>: 메서드 선언 앞에 필수 명시)
...
//      Pair.<Integer>print("안녕"); // incompatible types(호환되지 않는 타입)
//      : java.lang.String cannot be converted to java.lang.Integer

        Pair.<Integer>print(120); // 120
...
//      Pair<int, char> pair4 = new Pair<int, char>();
        ...
        // cf) 제네릭은 컴파일 이후 실제 타입 정보가 사라지고 Object로 변환
        //      - 런타임 시에는 Object로 동작
        //      => 타입 소가(Type Erasure)

        List<String> list = new ArrayList<>();
        list.add("Hello"); 
        // 컴파일 시 리스트 내부에 요소가 String 인지 검사
