...
// cf) 대표적인 보일러 플레이트 코드
// 보일러 플레이트 코드
// : 프로그램에서 반복되는 작업이나 패턴에 대한 일종의 표준화 된 코드
// - getter / setter
// - toString()
// - 생성자
// - 빌터 패턴
...
// - 주로 Maven 필드도구와 함께 사용되지만, Gradle 에서도 사용

// 2) gradle 리로드 (refresh)

// 3) 설정(빌드, 실행, 배포 => 컴파일러 => 어노테이션 처리 활성화)

// == 롬복(Lombok) 라이브러리 이노테이션 종류 == //
// 1. @Getter / Setter
...
// - 필드값 모두 메서드 체이닝으로 가져옴 (필수값 또한 메서드 체이닝으로 구성)
// - 메서드 체이닝 이름은 필드명
// => 필수값을 체이닝에서 누락 시 오류
